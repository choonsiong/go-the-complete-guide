...
import (
	"errors"
	"example.com/bank-file-package-folder/util"
	"fmt"
)
...
func menu() int {
	...
	fmt.Printf("Hi %s, What do you want to do?\n", username)
...
func readBalanceFromFile(filename string) (balance float64, err error) {
	return util.ReadFloatFromFile(filename)
}

func writeBalanceToFile(balance float64, filename string) error {
	return util.WriteFloatToFile(balance, filename)
}
